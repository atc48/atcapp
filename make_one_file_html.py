...
<script>
 $(function () {
   ...
   var atc = atcapp.boot("atcapp", "fix-search", "debugger", {assets_dir: "${ASSETS_DIR}", assets_base64_map: assetsBase64Map });
});
</script>
...
def main():
    ...
    js_minified = make_minified(js_list, (lambda s: s) if NO_COMPRESS else rjsmin.jsmin)
    html = html.replace("${JS_MINIFIED}", js_minified)

    css_list = make_filepath_list([CSS_ROOT_DIR], ".css")
    css_minified = make_minified(css_list, (lambda s: s) if NO_COMPRESS else rcssmin.cssmin)
    ...
    html = html.replace("${ASSETS_DIR}", ASSETS_DIR).replace("${ASSETS_BASE64_MAP}", json.dumps(assets_base64_map))
